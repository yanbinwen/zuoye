...
export function getRemoteMenu() {
  console.log("remoteMenu", remoteMenu)
  return remoteMenu;
}
...
function patchRouteItems(route: any, menu: any, parentPath: string) {
  for (const menuItem of menu) {
    if (menuItem.component === 'Layout' || menuItem.component === 'ParentView') {
      ...
      console.log("parentPath",parentPath + menuItem.path)
      const newRoute = {
...
// }
export async function getUserInfo(options?: Record<string, any>) {
  const res = await request<{ data: API.UserInfoResult }>('/api/getInfo', {
    method: 'GET',
    ...(options || {}),
  });
  return res.data;
...
export async function getRouters(): Promise<any> {
  ...
  const curpath = pathSegments.length > 0 ? pathSegments[0] : '';
  console.log("curpath", curpath)
  if (curpath === null || curpath === undefined || curpath === '' || curpath === '/') {
    return request('/api/getRouters');
  }else{
    return request('/api/getRouters?path='+curpath, { method: 'GET' });
  }
...
export function convertCompatRouters(childrens: API.RoutersMenuItem[]): any[] {
  return childrens.map((item: API.RoutersMenuItem) => {
    return {
      path: item.path,
      icon: createIcon(item?.meta?.icon??"AppstoreOutlined"),
      //  icon: item.meta.icon,
